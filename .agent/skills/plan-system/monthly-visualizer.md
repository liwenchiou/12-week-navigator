...
3.  **內容映射細節與完整度強制要求 (No Omission Policy)**：
    產出的網頁**必須完整包含以下所有區塊**，即使內容短少也不得省略：
    - **執行數據總覽 (Dashboard)**：包含 Score Board 與 12 週總進度條 (Total Blueprint Progressbar)。
    - **習慣穩定度分析 (Habits)**：表格下方必須動態顯示 **習慣備註 (Habit Note)**，不得遺漏缺失數據的原因紀錄。
    - **深度回顧與紀實 (Narratives)**：展示生活分類精華。
    - **偏差分析與進化對策 (Pivots)**：展示偏差分析與具體的進化對策。
    - **年度戰略關鍵獲益 (Gains)**：以動態磁貼呈現「三大獲益」。
