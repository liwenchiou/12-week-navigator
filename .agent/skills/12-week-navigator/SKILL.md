...
1. **主動詢問**：「偵測到 12-week-navigator 已加入，是否要將專屬工作流 (init, start, report-guide) 連結至系統目錄以啟用 `/` 指令選單？」
2. **執行掛載 (範例指令)**：
   ```bash
...
1. **任務確認**：讀取 `workspace/config/週計畫.md` 當週區塊，逐條詢問達成狀態。
2. **數據計算**：執行分 = (已達成 / 總項數) * 100。
3. **熱點圖**：橫軸一至日，縱軸核心習慣，繪製 ✅/❌ 矩陣。
4. **深度回顧**：打破時間線，依「主題」整合日誌原文。**🚫 禁 AI 評語**。
...
### 階段 A：數據彙整 (Reporter)
1. **跨週整合**：擷取全月週報的評分、阻礙、獲益與金句。
2. **目標比對**：逐條確認 `workspace/config/月計畫.md` 達成現況並回寫狀態。
3. **靈魂提案**：根據數據提煉 3-5 組「兩字核心標語 (Slogan)」。

### 階段 B：視覺化封裝 (Visualizer)
1. **設計介入**：**必須**先執行 `ui-ux-pro-max` 腳本取得配色指南。
2. **網頁生成**：產出單一 HTML (React 18 + Tailwind + Framer Motion)。
3. **欄位映射**：
   - 📊 執行數據總覽 (含 12 週總進度條)
   - 🗓️ 習慣達成分析 / 📖 深度生活回顧
   - 🚧 偏差分析與進化對策 / 💎 月度三大獲益
...
3. **局部更新策略 (Partial Update)**：優先使用 `replace_file_content` 修改檔案。
4. **沙盒隔離原則 (Sandbox)**：測試請求一律限制在 `workspace/sandbox/` 目錄。
5. **PDCA 螺旋**：所有報告必須包含「阻礙分析」與「下一步對策」。

---
*Portable Module v2.0 | Designed for High-Output Engineers*
