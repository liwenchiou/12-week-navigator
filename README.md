...
### 場景 A：每日夜間小結
> **User**: 「幫我紀錄今日習慣。」  
> **Agent**: 「好的，已讀取 `核心習慣.md`。今天喝水有超過 3000ml 嗎？」  
> **User**: 「有，今天全部習慣都達成了，另外今天完成 AI Skills 的初步測試。」  
> **產出**: 自動在 `log/daily/` 生成當日日誌，標註所有習慣為 ✅ 並記錄心得。

### 場景 B：週日晚間回顧
> **User**: 「產出本週週報。」  
> **Agent**: 「目前掃描到本週任務：『React 筆記』與『飛輪 2 次』尚未在日誌中標註，請問進度如何？」  
> **User**: 「React 筆記完成了，飛輪這週只做了一次。」  
> **產出**: 更新 `config/週計畫.md` 狀態，並在 `log/weekly/` 生成包含執行評分與感性回顧的週報。

### 場景 C：月度視覺化報告
> **User**: 「幫我產出 2 月份總結網頁。」  
> **Agent**: 「分析完成。二月核心標語建議為『守護與重構』。正在調用 UI/UX PRO MAX 引擎生成網頁...」  
> **產出**: 在 `reports/` 資料夾產出具備 Framer Motion 動效與 React 儀表板的互動式 HTML 檔案。
